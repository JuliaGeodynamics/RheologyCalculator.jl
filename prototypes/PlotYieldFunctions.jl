...
function PlotYieldFunction()
    ...
    τ = LinRange(0, 2.0e8, 1000)
    P = LinRange(-1.0e8, 5.0e8, 1500)
    ...
    contour!(ax1, P ./ 1.0e6, τ ./ 1.0e6, F, label = "Drucker-Prager", levels = [0], color = :black)
    ...
    contour!(ax1, P ./ 1.0e6, τ ./ 1.0e6, F, label = "Fairhurst", levels = [0], color = :blue)

    # Hyperbolic
    σt = 10.0e6
    F = @. sqrt((τ') .^ 2 + (C * cosd(ϕ) - σt * sind(ϕ))^2) - (P * sind(ϕ) + C * cosd(ϕ))
    contour!(ax1, P ./ 1.0e6, τ ./ 1.0e6, F, label = "Hyperbolic", levels = [0], color = :red)
    ...
    contour!(ax1, P ./ 1.0e6, τ ./ 1.0e6, F, label = "Mod. Cam-Clay", levels = [0], color = :green)
    ...
    contour!(ax1, P ./ 1.0e6, τ ./ 1.0e6, F, label = "Hoek-Brown", levels = [0], color = :purple)
