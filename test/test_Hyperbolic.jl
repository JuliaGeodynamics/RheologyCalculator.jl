...
t_v, τ, P = stress_time(c, vars_2D(7.0e-14, 0 * 7.0e-15), x, xnorm, others; ntime = 80, dt = 1.0e7)
@test mean(τ) ≈ 536901.6577557874
@test mean(P) ≈ 341487.7290700593
@test all(isfinite, τ)
@test all(isfinite, P)

t_v, τ, P = stress_time(c, vars_2D(7.0e-14, 7.0e-15), x, xnorm, others; ntime = 30, dt = 2.0e7)
