...
@testset "Mod. Cam-Clay" begin
    ...
    t_v, τ, P = stress_time(c, vars_2D(0 * 7.0e-14, -7.0e-15), x, xnorm, others; ntime = 1300, dt = 1.0e8)
    @test mean(τ) ≈ 0.0
    @test mean(P) ≈ 8.41662039460073e7
    @test all(isfinite, τ)
    @test all(isfinite, P)

    t_v, τ, P = stress_time(c, vars_2D(7.0e-14, -4.0e-15), x, xnorm, others; ntime = 700, dt = 1.0e8)
