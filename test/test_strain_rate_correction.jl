...
const dt = 1.0e10
const εII = 1.0e-14
const τ0_a = 2.0e6   # first backstress
const τ0_b = 1.0e6   # second backstress
...
@testset "_η_KV" begin
    ...
    @test _η_KV((visc2, el1), (), args_base) ≈ η_v2 + η_el1  rtol = 1.0e-12
    ...
    @test _η_KV((visc2,), (sub1,), args_base) ≈ η_v2 + η_eff_M  rtol = 1.0e-12
    ...
    @test _η_KV((), (sub1, sub2), args_base) ≈ η_eff_M1 + η_eff_M2  rtol = 1.0e-12
end
...
@testset "_weighted_backstress" begin
    ...
    τ0 = (τ0_a,)
    @test only(_weighted_backstress((visc2, el1), (), εII, τ0, args_base, 0)) ≈ τ0_a  rtol = 1.0e-12
    ...
    τ0_sub = (τ0_a,)
    @test only(_weighted_backstress((visc2,), (sub1,), εII, τ0_sub, args_base, 0)) ≈ η_star * τ0_a  rtol = 1.0e-12
    ...
    @test only(_weighted_backstress((el1,), (sub1b,), εII, τ0_two, args_base, 0)) ≈ ws_expected  rtol = 1.0e-12

    # Non-zero el_idx_start (a leaf elastic already consumed offset=1):
    # τ0[1] belongs to a previous leaf; this branch uses τ0[2]
    τ0_offset = (0.0, τ0_a)
    @test only(_weighted_backstress((visc2, el1), (), εII, τ0_offset, args_base, 1)) ≈ τ0_a  rtol = 1.0e-12
end
...
@testset "effective_strain_rate_correction — Maxwell leaf" begin
    ...
    @test only(cor) ≈ expected  rtol = 1.0e-12
...
@testset "effective_strain_rate_correction — two parallel branches" begin
    ...
    cor = effective_strain_rate_correction(c, ε, τ0, others)
    @test only(cor) ≈ cor1 + cor2  rtol = 1.0e-12
end
