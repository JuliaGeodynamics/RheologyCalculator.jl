# `_bt_line_search_result`'s `(α, x_next, r, er)` must stay internally consistent.
using Test, StaticArrays
using RheologyCalculator.RheologyModels
import RheologyCalculator: SeriesModel, ParallelModel, initial_guess_x,
    normalisation_x, solve, compute_residual, mynorm, _bt_line_search_result, jacobian,
    backsolve, second_invariant_value, _direct_leaf_elastic_correction

@testset "line search convergence" begin
    ...
    @testset "the returned tuple is self-consistent" begin
        x = x0
        for _ in 1:5
            ...
            α, x_next, r_next, er_next = _bt_line_search_result(
            ...
            @test er_next == mynorm(r_next, xnorm)
