...
@testset "VEPCap Model " begin
    ...
    t_v2, τ2, P2 = stress_time(c, vars_2D(7.0e-14, 0 * 7.0e-15), x, xnorm, others; ntime = 80, dt = 1.0e7)
    @test mean(τ2) ≈ 551452.3775990973
    @test mean(P2) ≈ 305322.99751051754
    @test all(isfinite, τ2)
    @test all(isfinite, P2)

    t_v3, τ3, P3 = stress_time(c, vars_2D(7.0e-14, 7.0e-15), x, xnorm, others; ntime = 30, dt = 2.0e7)
