...
function Stokes2D_VEP(n)
    ...
    sc = (σ = 1.0e6, t = 1.0e10, L = 1.0e3)


    # Physics
    Lx, Ly = 2.0e3 / sc.L, 1.0e3 / sc.L   # domain size
    radi = 0.1e3 / sc.L           # inclusion radius
    η0 = 1.0e23 / sc.σ / sc.t       # viscous viscosity
    ηi = 1.0e10 / sc.σ / sc.t       # min/max inclusion viscosity
    G = 3.0e10 / sc.σ
    C = 5.0e7 / sc.σ
    Δt = 4.0e10 / sc.t
    εbg = -1.0e-15 * sc.t      # background strain-rate
    comp = true
    K = 5.0e10 / sc.σ
    ϕ = 30.0
    ψ = 10.0
    ηvp = 2.0e20 / sc.σ / sc.t

    # Maxwell viscoelastic model
    # elastic --- viscous - viscoplastic
    viscous_matrix = LinearViscosity(1.0e7)
    viscous_inclusion = LinearViscosity(1.0e-6)
    ...
    plastic = DruckerPragerCap(; C = 1.0e6, ϕ = 30.0, ψ = 10.0, η_vp = viscous_reg, Pt = -5.0e5)
    ...
    rel_drop = 1.0e-1         # relative drop of velocity residual per PH iteration
    ...
    for it in 1:nt
        ...
        iter = 0; err = 2 * ϵ; err_evo_it .= 0.0; err_evo_V .= 0.0; err_evo_P .= 0.0
        @time for itPH in 1:100
            # Boundaries
            ...
            if itPH == 1
                errVx0 = errVx; errVy0 = errVy; errPt0 = errPt
            end
    ...
    dark_axis!(ax2)
    hm2 = heatmap!(ax2, xc, yc, (Pt .* sc.σ) ./ 1.0e6, colormap = :vikO)
    xlims!(ax2, -Lx / 2, Lx / 2)
    ...
    dark_line_axis!(ax3)
    lines!(ax3, (Tii_evo .* sc.σ) ./ 1.0e6, color = :deepskyblue2, linewidth = 3)
