...
`x` is the starting solver vector: from [`initial_guess_x`](@ref), or the
[`RCSolution`](@ref) of a previous solve, e.g. the last time step, which reuses
every entry and is usually the faster start. `vars`
