...
"""
inspect(c::AbstractCompositeModel) = inspect(generate_equations(c))

@generated function inspect(eqs::NTuple{N, CompositeEquation}) where {N}
    return quote
        @inline
        e = Base.@ntuple $N i -> _inspect(eqs[i])
        superflatten(e)
    end
end
...
@inline function _inspect(eq::CompositeEquation)
