...
c, x, τ0, ε, args, others = let
    ...
    viscous = LinearViscosity(1.0e31)
    elastic = IncompressibleElasticity(10.0e9)
    viscous2 = LinearViscosity(1.0e18)
    elastic2 = IncompressibleElasticity(5.0e8)
    ...
    others = (; dt = 1.0e10, τ0 = (0.0e0,), P0 = (0.0,)) # other non-differentiable variables needed to evaluate the state functions

    x = initial_guess_x(c, vars, args, others)
    τ0 = ([0.0, 0.0e3, 0.0], [1.0e5, -1.0e5, 9.0e5])  # initial stress state (2D: xx,zz,xz)
    ε = [1.0e-14, -1.0e-14, 1.0e-15]         # applied dev. strainrates (xx,zz,xz)
...
function stress_time_full_tensor(c, x, τ0, ε; ntime = 200, dt = 1.0e8)
    ...
    for i in 2:ntime
        ...
        vars = (; ε = ε[1], θ = 0.0)
        x = solve(c, x, vars, others, verbose = true, atol = 1.0e-15, rtol = 1.0e-11, itermax = 100)
        ...
        vars = (; ε = ε[3], θ = 0.0)
        x = solve(c, x, vars, others, verbose = false, atol = 1.0e-15, rtol = 1.0e-11, itermax = 100)
...
fig, ax, li = scatter(t_v[2:end] / SecYear, τII_tot[2:end] / 1.0e6, color = :blue, label = "numerics, full tensor")
lines!(ax, t_v[2:end] / SecYear, τII_1_tot[2:end] / 1.0e6, color = :magenta, label = "analytics, elastic1", linewidth = 3)
lines!(ax, t_v[2:end] / SecYear, τII_2_tot[2:end] / 1.0e6, color = :orange, label = "analytics, elastic2", linewidth = 3)

lines!(ax, t_v[2:end] / SecYear, τII_ana[2:end] / 1.0e6, color = :red, label = "analytics", linewidth = 3)
lines!(ax, t_v[2:end] / SecYear, τII_invariants[2:end] / 1.0e6, color = :green, label = "invariants, manually", linewidth = 3)
lines!(ax, t_v[2:end] / SecYear, τII_invariants_2[2:end] / 1.0e6, color = :yellow, label = "invariants, elastic2", linewidth = 3)
