...
c, x, τ0, ε, args, others = let
    ...
    elastic = IncompressibleElasticity(5.0e10)
...
function stress_time_full_tensor(c, x, τ0, ε; ntime = 200, dt = 1.0e8)
    ...
    for i in 2:ntime
        ...
        vars = (; ε = ε[1], θ = 0.0)
        x = solve(c, x, vars, others, verbose = true, atol = 1.0e-15, rtol = 1.0e-11, itermax = 10)
        ...
        vars = (; ε = ε[2], θ = 0.0)
        x = solve(c, x, vars, others, verbose = false, atol = 1.0e-15, rtol = 1.0e-11, itermax = 10)
...
function stress_time_invariant_manual_parallel(c, x, τ0, ε; ntime = 200, dt = 1.0e8)
    ...
    for i in 2:ntime
        ...
        x = solve(c, x, vars, others, verbose = true, atol = 1.0e-15, rtol = 1.0e-15, itermax = 10)
...
fig, ax, li = scatter(t_v[2:end] / SecYear, τII_tot[2:end] / 1.0e6, color = :blue, label = "numerics, full tensor", linewidth = 3)
