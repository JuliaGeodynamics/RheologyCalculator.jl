...
using RheologyCalculator.RheologyModels: Golchin, compute_F, compute_Q
...
c, x, xnorm, vars, args, others = let
    ...
    plastic = Golchin(; M = 0.9, N = 0.5, γ = 0.5, α = 0.5, β = 0.0, Pt = -1.0e5, Pc = 1.3e8, η_vp = 1.0e20)
    ...
    char_τ = 1.0e25 # Issue with scaling
...
function figure()
    ...
    @show F1
    println("-------")
    t_v2, τ2, P2, F2, mode2_2 = stress_time(c, vars_2D(0 * 7.0e-14, -7.0e-15), x, xnorm, others; ntime = 1300, dt = 1.0e8)
    println("-------")
    t_v3, τ3, P3, F3, mode2_3 = stress_time(c, vars_2D(7.0e-14, -4.0e-15), x, xnorm, others; ntime = 700, dt = 1.0e8)
    println("-------")
    ...
    GLMakie.contour!(ax4, P / 1.0e6, τII / 1.0e6, F, levels = [0.001], color = :black)
    GLMakie.contour!(ax4, P / 1.0e6, τII / 1.0e6, Q, levels = [0.001], color = :black, linestyle = :dash)
    GLMakie.scatter!(ax4, P2 / 1.0e6, τ2 / 1.0e6, color = :red, label = L"1")
    GLMakie.scatter!(ax4, P3 / 1.0e6, τ3 / 1.0e6, color = :blue, label = L"2")
    GLMakie.scatter!(ax4, P1 / 1.0e6, τ1 / 1.0e6, color = :green, label = L"3")
