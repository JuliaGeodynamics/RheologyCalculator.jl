...
function run_rheologycalculator(setup; ntime, dt)
    return RC.simulate_crustal_creep_3D(
        setup.c,
        setup.diffusion,
        setup.dislocation,
        setup.vars,
...
function run_newton(setup; ntime, dt)
    return NR.simulate_crustal_creep_newton_3D(
        setup.diffusion,
        setup.dislocation,
        setup.elastic,
        setup.vars,
...
function main(; ntime = 501, dt = 250 * RC.SecYear, samples = 20, warmups = 3)
    rc_setup = RC.setup_crustal_creep_3D()
    nr_setup = NR.setup_crustal_creep_newton_3D()

    println("Comparing 3D crustal elastic + diffusion + dislocation examples")
    ...
    speedup = median(nr.times) / median(rc.times)
