...
using RheologyCalculator.RheologyModels: second_invariant_2D, vars_2D, stress_tensor_from_invariant_2D, elastic_stress_history_2D
...
using RheologyCalculator.RheologyModels: RateStateFriction, update_Ω, compute_strain_rate
...
"""
function compute_dt_ratestate(rs::RateStateFriction, re::AbstractElasticity, P, Vp, Ω; dt_min = 1.0e-4, dt_max = 1.0e6, f_courant = 1.0e-3)
...
"""
dt_courant(Vmax, Δ; f = 1.0e-3) = f * Δ / Vmax
...
function stress_time(c, vars, x, xnorm, others; ntime = 200, dt = 1.0e6, dt_max = 1.0e6, dt_min = 1.0e-4)
    ...
    t = 0.0
    for i in 2:ntime
        others = (; dt = dt, τ0 = τ_e, Ω_old = Ω, P0 = P_e, P = (50.0e6,))       # other non-differentiable variables needed to evaluate the state functions
        ...
        dt = compute_dt_ratestate(c[1], c[2], others.P[1], Vp, Ω, dt_min = 1.0e-4, dt_max = 1.0e6)
    end
...
c, x, xnorm, vars, args, others = let
    ...
    others = (; dt = 1.1e-2, τ0 = (stress_tensor_from_invariant_2D(2.0e7, vars.ε),), P0 = (0.0,), P = (50.0e6,)) # other non-differentiable variables needed to evaluate the state functions
...
t_v, τ, Ω_v, V_v, τy = stress_time(c, vars, x, xnorm, others; ntime = 1000, dt = 1.0e6)
...
lines!(ax1, t_v / SecYear, τy / 1.0e6, color = :red, label = L"\tau_{interface}")
lines!(ax1, t_v / SecYear, τ / 1.0e6, color = :blue, label = L"\tau_{II}")
scatter!(ax1, t_v / SecYear, τ / 1.0e6, color = :blue)
axislegend(ax1, position = :rb, labelsize = 18)

ax2 = Axis(fig[1, 2], xlabel = L"$t$ [yr]", ylabel = L"$\Omega$")
ax2r = Axis(
    fig[1, 2]; ylabel = L"$V_p$ [m/s]", yaxisposition = :right,
